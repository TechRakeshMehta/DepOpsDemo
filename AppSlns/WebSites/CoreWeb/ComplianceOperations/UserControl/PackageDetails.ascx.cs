...
{
    public partial class PackageDetails : BaseUserControl, IPackageDetailsView
    {
        private PackageDetailsPresenter _presenter = new PackageDetailsPresenter();
        ...
        public IPackageDetailsView CurrentViewContext
        {
            get { return this; }
        }
        ...
        #endregion
        #region UAT-3601
        String IPackageDetailsView.PackageDetailHeaderLabel
        {
            set
            {
        ...
        String IPackageDetailsView.ChangePackageSelectionButtonLabel
        {
            set
            {
        ...
        private void BindPackageDetails()
        {
                            ...
                            if (CurrentViewContext.SelectedPackageDetails[cptype].DeptProgramPackage.CompliancePackage.CompliancePackageID == _packageId)//UAT-3283
                            {
                                ...
                                if (ctrlLblPackage.IsNotNull())
                                    ctrlLblPackage.Text = (String.IsNullOrEmpty(CurrentViewContext.SelectedPackageDetails[cptype].DeptProgramPackage.CompliancePackage.PackageLabel)
                                        ...
                                        : CurrentViewContext.SelectedPackageDetails[cptype].DeptProgramPackage.CompliancePackage.PackageLabel).HtmlEncode();
            ...
            if (bkgHierarchyMappingIds.IsNotNull() && bkgHierarchyMappingIds.Count > AppConsts.NONE)
            {
                ...
                if (String.Compare(Convert.ToString(applicantOrderCart.OrderRequestType), OrderRequestType.CompleteOrderByApplicant.GetStringValue(), true) != AppConsts.NONE)
                {
                    Presenter.GetOrderBkgPackageDetails(bkgHierarchyMappingIds,applicantOrderCart.SelectedHierarchyNodeID);
                }
                ...
                if (!this.lstPackagePrices.IsNullOrEmpty())
                {
                    foreach (var pkg in CurrentViewContext.lstExternalPackages)
                    {
                        ...
                        if (_pkgPricingData.IsNotNull() && !pkg.IsInvoiceToInstitutionType)       //UAT: 4467                  
                ...
                if (!lstBkgOrderSvcLineData.IsNullOrEmpty())
                {
                    ...
                    List<PackageServiceItemContract> packageServiceItemContractForInvoiceTypeList = CurrentViewContext.lstExternalPackages.IsNotNull() ?  CurrentViewContext.lstExternalPackages.Where(xx => xx.IsInvoiceOnlyAtPackageLevel == true)
                                                .Select(x => x.ListPackagePaymentContract).ToList() : new List<PackageServiceItemContract>();

                    // Additional fee line Items for any type of invoice not to be shown.
                    List<BackroundOrderServiceLinePrice> lstBkgOrderSvcLineDataFilter = new List<BackroundOrderServiceLinePrice>();                     
                    List<Int32> listBackgroundServiceIds = new List<Int32>(); //get list of Background service ids

                    if (!packageServiceItemContractForInvoiceTypeList.IsNullOrEmpty() && packageServiceItemContractForInvoiceTypeList.Count> 0)
                    {
                        foreach (var item in packageServiceItemContractForInvoiceTypeList)
                        {
                            if (item.BackgroundServiceID.IsNotNull() && item.BackgroundServiceID.Count > 0)
                            {
                                listBackgroundServiceIds.AddRange(item.BackgroundServiceID);
                            }
                        }
                    }
                    if (listBackgroundServiceIds.IsNotNull() && listBackgroundServiceIds.Count> 0)
                    {
                        lstBkgOrderSvcLineDataFilter = lstBkgOrderSvcLineData.Where(xx => !listBackgroundServiceIds.Contains(xx.BackgroundServiceID)).ToList();
                        grdOrderServiceLinePriceInfo.DataSource = lstBkgOrderSvcLineDataFilter;
                        SetOrderServiceLinePriceInfoGridVisiblity(lstBkgOrderSvcLineDataFilter);// UAT:4467
                    }

                    //grdOrderServiceLinePriceInfo.DataSource = lstBkgOrderSvcLineDataFilter.IsNotNull() && lstBkgOrderSvcLineDataFilter.Count > 0
                    //                                            ? lstBkgOrderSvcLineDataFilter : lstBkgOrderSvcLineData;
                    // End UAT: 4467

                    // UAT: 4467:- Commented to filter data for any payment either Invoice to Institution or Invoice for Approval or both.
                    else
                    {
                        grdOrderServiceLinePriceInfo.DataSource = lstBkgOrderSvcLineData;
                        SetOrderServiceLinePriceInfoGridVisiblity(lstBkgOrderSvcLineData); //UAT:4467
                    }                    
            ...
            List<BackgroundPackagesContract> nonInvoicePackages = new List<BackgroundPackagesContract>();
            if (CurrentViewContext.lstExternalPackages.IsNotNull())
            {
                nonInvoicePackages = CurrentViewContext.lstExternalPackages.Where(xx => xx.IsInvoiceOnlyAtPackageLevel == false).ToList();
            }                        

            if ((CurrentViewContext.lstExternalPackages.IsNotNull() && CurrentViewContext.lstExternalPackages.Any(cond => cond.IsInvoiceOnlyAtPackageLevel == true) && nonInvoicePackages.Count == 0) || isCompliancePackageOfInvoiceType)
            {
        ...
        /// <summary>
        /// Set Order ServiceLine Price Info Grid Visiblity [UAT:4467]
        /// </summary>
        /// <param name="lstBkgOrderSvcLineDataFilter"></param>
        private void SetOrderServiceLinePriceInfoGridVisiblity(List<BackroundOrderServiceLinePrice> lstBkgOrderSvcLineDataFilter)
        {
            if (!lstBkgOrderSvcLineDataFilter.Any())
            {
                dvTotalPrice.Style.Add("display", "none");
                divBkgSvcBreakdwnFees.Attributes.Add("style", "display:none");
            }
            ...
            {
                dvTotalPrice.Style.Add("display", "block");
                divBkgSvcBreakdwnFees.Attributes.Add("style", "display:block");
            }
        }
        ...
        public PackageDetailsPresenter Presenter
        {
            get
            {
