...
(function ($) {
    ...
    function SignaturePad(selector, options) {
         ...
         * @type {Object}
         */
        , settings = $jQuery.extend({}, $jQuery.fn.signaturePad.defaults, options)
         ...
         * @type {Object}
         */
        , canvas = $jQuery(settings.canvas, context)
        ...
        function stopDrawing(e) {
            ...
            if (settings.output && output.length > 0)
                $jQuery(settings.output, context).val(JSON.stringify(output))
        }
        ...
        function clearCanvas() {
            ...
            $jQuery(settings.output, context).val('')
            output = []
        ...
        function disableCanvas() {
            ...
            $jQuery(document).unbind('mouseup.signaturepad')
            canvas.unbind('mousedown.signaturepad')
            canvas.unbind('mousemove.signaturepad')
            canvas.unbind('mouseleave.signaturepad')

            $jQuery(settings.clear, context).unbind('click.signaturepad')
        }
        ...
        function initDrawEvents(e) {
            ...
            if (touchable) {
                ...
                $jQuery(document).bind('mouseup.signaturepad', function () {
                    if (mouseButtonDown) {
                        stopDrawing()
         ...
         */
        function drawIt() {
            $jQuery(settings.typed, context).hide()
            clearCanvas()
            ...
            $jQuery(settings.clear, context).bind('click.signaturepad', function (e) { e.preventDefault(); clearCanvas() })

            $jQuery(settings.typeIt, context).bind('click.signaturepad', function (e) { e.preventDefault(); typeIt() })
            $jQuery(settings.drawIt, context).unbind('click.signaturepad')
            $jQuery(settings.drawIt, context).bind('click.signaturepad', function (e) { e.preventDefault() })

            $jQuery(settings.typeIt, context).removeClass(settings.currentClass)
            $jQuery(settings.drawIt, context).addClass(settings.currentClass)
            $jQuery(settings.sig, context).addClass(settings.currentClass)

            $jQuery(settings.typeItDesc, context).hide()
            $jQuery(settings.drawItDesc, context).show()
            $jQuery(settings.clear, context).show()
        }
        ...
        function typeIt() {
            clearCanvas()
            disableCanvas()
            $jQuery(settings.typed, context).show()

            $jQuery(settings.drawIt, context).bind('click.signaturepad', function (e) { e.preventDefault(); drawIt() })
            $jQuery(settings.typeIt, context).unbind('click.signaturepad')
            $jQuery(settings.typeIt, context).bind('click.signaturepad', function (e) { e.preventDefault() })

            $jQuery(settings.output, context).val('')

            $jQuery(settings.drawIt, context).removeClass(settings.currentClass)
            $jQuery(settings.typeIt, context).addClass(settings.currentClass)
            $jQuery(settings.sig, context).removeClass(settings.currentClass)

            $jQuery(settings.drawItDesc, context).hide()
            $jQuery(settings.clear, context).hide()
            $jQuery(settings.typeItDesc, context).show()

            typeItCurrentFontSize = typeItDefaultFontSize = $jQuery(settings.typed, context).css('font-size').replace(/px/, '')
        }
         ...
         */
        function type(val) {
            var typed = $jQuery(settings.typed, context)
        ...
        function onBeforeValidate(context, settings) {
            $jQuery('p.' + settings.errorClass, context).remove()
            context.removeClass(settings.errorClass)
            $jQuery('input, label', context).removeClass(settings.errorClass)
        }
        ...
        function onFormError(errors, context, settings) {
            if (errors.nameInvalid) {
                ...
                $jQuery(settings.name, context).focus()
                $jQuery(settings.name, context).addClass(settings.errorClass)
                $jQuery('label[for=' + $jQuery(settings.name).attr('id') + ']', context).addClass(settings.errorClass)
            }
        ...
        function validateForm() {
            ...
            if ($jQuery(settings.name, context).val() === '') {
        ...
        function init() {
            ...
            if (parseFloat(((/CPU.+OS ([0-9_]{3}).*AppleWebkit.*Mobile/i.exec(navigator.userAgent)) || [0, '4_2'])[1].replace('_', '.')) < 4.1) {
                $jQuery.fn.Oldoffset = $jQueryfn.offset;
            ...
            $jQuery(settings.typed, context).bind('selectstart.signaturepad', function (e) { return $jQuery(e.target).is(':input') })
            canvas.bind('selectstart.signaturepad', function (e) { return $jQuery(e.target).is(':input') })
            ...
            if (element.getContext) {
                ...
                $jQuery(settings.sig, context).show()

                if (!settings.displayOnly) {
                    if (!settings.drawOnly) {
                        $jQuery(settings.name, context).bind('keyup.signaturepad', function () {
                            type($jQuery(this).val())
                        })

                        $jQuery(settings.name, context).bind('blur.signaturepad', function () {
                            type($jQuery(this).val())
                        })

                        $jQuery(settings.drawIt, context).bind('click.signaturepad', function (e) {
                            e.preventDefault()
                            drawIt()
                        })
                    }
                    ...
                    if (settings.validateFields) {
                        if ($jQuery(selector).is('form')) {
                            $jQuery(selector).bind('submit.signaturepad', function () { return validateForm() })
                        } else {
                            $jQuery(selector).parents('form').bind('submit.signaturepad', function () { return validateForm() })
                        }
                    }

                    $jQuery(settings.sigNav, context).show()
                }
            }
        }
        ...
        $jQuery.extend(self, {
          ...
          , regenerate: function (paths) {
              ...
              if (settings.output && $jQuery(settings.output, context).length > 0)
                  $jQuery(settings.output, context).val(JSON.stringify(output))
          }
    ...
    $jQuery.fn.signaturePad = function (options) {
        ...
        this.each(function () {
            if (!$jQuery.data(this, 'plugin-signaturePad')) {
                api = new SignaturePad(this, options)
                api.init()
                $jQuery.data(this, 'plugin-signaturePad', api)
            } else {
                api = $jQuery.data(this, 'plugin-signaturePad')
     ...
     * @type {Object}
     */
    $jQuery.fn.signaturePad.defaults = {
