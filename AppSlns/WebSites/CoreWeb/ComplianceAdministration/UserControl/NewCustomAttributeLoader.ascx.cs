...
{
    public partial class NewCustomAttributeLoader : BaseUserControl, ICustomAttributeLoaderView
    {
        #region Variables
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                //divForm.Attributes.Add("class", "row bgLightGreen");
        ...
        {
            Control customAttributeRow = Page.LoadControl("~\\ComplianceAdministration\\UserControl\\NewCustomAttributeRowControl.ascx");

            (customAttributeRow as NewCustomAttributeRowControl).lstTypeCustomAttributes = lstAttributes;
            (customAttributeRow as NewCustomAttributeRowControl).SelectedRecordId = this.ValueRecordId;
            (customAttributeRow as NewCustomAttributeRowControl).IsReadOnly = this.IsReadOnly;
            (customAttributeRow as NewCustomAttributeRowControl).ControlDisplayMode = this.ControlDisplayMode;

            #region UAT 1438: Enhancement to allow students to select a User Group.

            (customAttributeRow as NewCustomAttributeRowControl).TenantID = this.TenantId;
            (customAttributeRow as NewCustomAttributeRowControl).OrganizationUserId = this.CurrentLoggedInUserId;
            (customAttributeRow as NewCustomAttributeRowControl).ShowReadOnlyUserGroupCustomAttribute = this.ShowReadOnlyUserGroupCustomAttribute;
            (customAttributeRow as NewCustomAttributeRowControl).lstUserGroups = this.lstUserGroups;
            (customAttributeRow as NewCustomAttributeRowControl).lstUserGroupsForUser = this.lstUserGroupsForUser;
            #endregion

            if (!String.IsNullOrEmpty(this.ValidationGroup))
                (customAttributeRow as NewCustomAttributeRowControl).ValidationGroup = this.ValidationGroup;

            if (this.AttributeValues.IsNotNull() && AttributeValues.Count() > 0)
                (customAttributeRow as NewCustomAttributeRowControl).AttributeValues = this.AttributeValues;
        ...
        private void AddDataToList(object ctrl, String attributeValue, String attributeTypeCode, String lblText)
        {
            ...
            Control ucAttributes = (ctrl as NewCustomAttributeControl) as Control;
        ...
        public List<TypeCustomAttributes> GetCustomAttributeValues()
        {
            foreach (Control rowControl in pnlRows.Controls)
            {
                ...
                if (rowControl.GetType().BaseType == typeof(NewCustomAttributeRowControl))
                {
                    foreach (var attributeControlPanel in rowControl.Controls)
                    {
                        if (attributeControlPanel.GetType() == typeof(Panel))
                        {
                            foreach (var ctrl in (attributeControlPanel as Panel).Controls)
                            {
                                ...
                                if (ctrl.GetType().BaseType == typeof(NewCustomAttributeControl))
                                {
                                    HtmlContainerControl attrDiv = ((ctrl as NewCustomAttributeControl).FindControl("divControlMode") as HtmlContainerControl);
                                    Label lblLabel = ((ctrl as NewCustomAttributeControl).FindControl("lblLabel") as Label);
