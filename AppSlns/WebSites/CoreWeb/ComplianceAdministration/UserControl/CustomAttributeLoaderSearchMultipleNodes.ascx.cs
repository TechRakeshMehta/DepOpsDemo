...
{
    public partial class CustomAttributeLoaderSearchMultipleNodes : BaseUserControl, ICustomAttributeLoaderSearchMultipleNodesView
    {
        #region Variables

        #region Private Variables

        private CustomAttributeLoaderSearchMultipleNodesPresenter _presenter = new CustomAttributeLoaderSearchMultipleNodesPresenter();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!CurrentViewContext.nodeLable.IsNullOrEmpty())
            {
                System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "SetFocusInstitutionLink();", true);
            }
            if (TenantId != 0)
            {
                Presenter.GetCustomAttributes(CurrentViewContext.DPM_ID, CustomAttributeUseTypeContext.Hierarchy.GetStringValue(), CurrentViewContext.TenantId, hdnScreenType.Value);

                #region UAT 1438: Enhancement to allow students to select a User Group. Check where page is loaded from orderflow or not.

                if (!ShowUserGroupCustomAttribute)
                {
                    //Removes the User Group Custom Attribute Type.
        ...
        {
            Control customAttributeRow = Page.LoadControl("~\\ComplianceAdministration\\UserControl\\CustomAttributeRowControlSearchMultipleNodes.ascx");
            (customAttributeRow as CustomAttributeRowControlSearchMultipleNodes).lstTypeCustomAttributes = lstAttributes;
            (customAttributeRow as CustomAttributeRowControlSearchMultipleNodes).previousValues = CurrentViewContext.previousValues;
        ...
        private void AddDataToList(object ctrl, String attributeValue, String attributeTypeCode, String lblText)
        {
            ...
            Control ucAttributes = (ctrl as CustomAttributeControlSearchMultipleNodes) as Control;
        ...
        public List<TypeCustomAttributesSearch> GetCustomAttributeValues()
        {
            foreach (Control rowControl in pnlRows.Controls)
            {
                ...
                if (rowControl.GetType().BaseType == typeof(CustomAttributeRowControlSearchMultipleNodes))
                {
                    foreach (var attributeControlPanel in rowControl.Controls)
                    {
                        if (attributeControlPanel.GetType() == typeof(Panel))
                        {
                            foreach (var ctrl in (attributeControlPanel as Panel).Controls)
                            {
                                ...
                                if (ctrl.GetType().BaseType == typeof(CustomAttributeControlSearchMultipleNodes))
                                {
                                    HtmlContainerControl attrDiv = ((ctrl as CustomAttributeControlSearchMultipleNodes).FindControl("divControlMode") as HtmlContainerControl);
                                    Label lblLabel = ((ctrl as CustomAttributeControlSearchMultipleNodes).FindControl("lblLabel") as Label);
        ...
        public CustomAttributeLoaderSearchMultipleNodesPresenter Presenter
        {
            get
            {
        ...
        public ICustomAttributeLoaderSearchMultipleNodesView CurrentViewContext
        {
            get { return this; }
        }
        ...
        public String IsTreeHierachyChanged
        {
            get
            {
                return hdnIsTreeHierarchyChanged.Value;
            }
            set
            {
                hdnIsTreeHierarchyChanged.Value = "";
            }
        }
        ...
        /// <summary>
        /// CSV of the multiple DPM nodes selected from the hierarchy treeview with checkboxes - UAT 1055
        /// </summary>
        public String DPM_ID
        {
            get
            {
                return hdnDepartmntPrgrmMppng.Value.IsNullOrEmpty() ? String.Empty : hdnDepartmntPrgrmMppng.Value;
            }
            set
            {
        ...
        /// <summary>
        /// NodeID's of all the selected nodes from the hierarchy treeview with checkboxes - UAT 1055
        /// </summary>
        public String NodeIds
        {
            get
            {
                return hdnInstitutionNodeId.Value.IsNullOrEmpty() ? String.Empty : hdnInstitutionNodeId.Value;
            }
            set
            {
        ...
        public String ScreenType
        {
            get
            {
                return Convert.ToString(ViewState["ScreenType"]);
            }
            set
            {
                ViewState["ScreenType"] = value.ToString();
                hdnScreenType.Value = Convert.ToString(value);
            }
        }

        //UAT 1438: Enhancement to allow students to select a User Group. Check where page is loaded from orderflow or not.
        public Boolean ShowUserGroupCustomAttribute { get; set; }
        ...
        {
            ClearControlsConditionally(tenantId, resetTenant, false);
        }

        private void ClearControlsConditionally(Int32? tenantId, Boolean resetTenant, Boolean IgnoreTenantCheck)
        {
            if (pnlRows.Controls.Count > 0 && (IgnoreTenantCheck || tenantId.IsNullOrEmpty()))
            {
        ...
        public void ResetControlData(Boolean resetHierarchyTree = false)
        {
            foreach (var control in pnlRows.Controls)
            {
                if (control.GetType().FullName.Contains("customattributerowcontrolsearchmultiplenodes"))
                {
                    ((CustomAttributeRowControlSearchMultipleNodes)control).ResetCustomAttributes();
                }
            }

            if (resetHierarchyTree)
            {
                ClearControlsConditionally(TenantId, true, true);
