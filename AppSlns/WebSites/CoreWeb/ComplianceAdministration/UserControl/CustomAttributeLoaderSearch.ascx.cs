...
{
    public partial class CustomAttributeLoaderSearch : BaseUserControl, ICustomAttributeLoaderSearchView
    {
        #region Variables
        ...
        {
            Control customAttributeRow = Page.LoadControl("~\\ComplianceAdministration\\UserControl\\CustomAttributeRowControlSearch.ascx");
            (customAttributeRow as CustomAttributeRowControlSearch).lstTypeCustomAttributes = lstAttributes;
            (customAttributeRow as CustomAttributeRowControlSearch).previousValues = CurrentViewContext.previousValues;
        ...
        private void AddDataToList(object ctrl, String attributeValue, String attributeTypeCode, String lblText)
        {
            ...
            Control ucAttributes = (ctrl as CustomAttributeControlSearch) as Control;
        ...
        public List<TypeCustomAttributesSearch> GetCustomAttributeValues()
        {
            foreach (Control rowControl in pnlRows.Controls)
            {
                ...
                if (rowControl.GetType().BaseType == typeof(CustomAttributeRowControlSearch))
                {
                    foreach (var attributeControlPanel in rowControl.Controls)
                    {
                        if (attributeControlPanel.GetType() == typeof(Panel))
                        {
                            foreach (var ctrl in (attributeControlPanel as Panel).Controls)
                            {
                                ...
                                if (ctrl.GetType().BaseType == typeof(CustomAttributeControlSearch))
                                {
                                    HtmlContainerControl attrDiv = ((ctrl as CustomAttributeControlSearch).FindControl("divControlMode") as HtmlContainerControl);
                                    Label lblLabel = ((ctrl as CustomAttributeControlSearch).FindControl("lblLabel") as Label);
