...
{
    public partial class StudentClientAdminDataAuditHistory : BaseUserControl, IApplicantDataAuditHistoryView
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            try
            {
                ...
                base.OnInit(e);
                base.Title = "Student and Admin Data Audit History";
                base.SetPageTitle("Student and Admin Data Audit History");
            }
        ...
        protected void grdApplicantDataAudit_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                ...
                if (cmbRole.CheckedItems.Count() > 0)
                {
                    ...
                    selectedFltrRoleNames = String.Join(",", fltrRoleNames);
                }
                else 
                {
                    var fltrRoleList = Enum.GetValues(typeof(StudentClientAdminDataAuditHistoryRole)).Cast<StudentClientAdminDataAuditHistoryRole>().Select(x => x.GetStringValue().Replace(" ", "")).ToList();
                    selectedFltrRoleNames = String.Join(",", fltrRoleList);
                }
        ...
        {
            var fltrRoleList = Enum.GetValues(typeof(StudentClientAdminDataAuditHistoryRole)).Cast<StudentClientAdminDataAuditHistoryRole>().Select(x => new KeyValuePair<Int32, String>(Convert.ToInt32(x), x.GetStringValue())).ToList();
