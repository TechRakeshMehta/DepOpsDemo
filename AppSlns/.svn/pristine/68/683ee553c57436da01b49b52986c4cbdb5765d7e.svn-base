...
{
    public partial class SharedUserRequirementItemForm : BaseUserControl, IRequirementItemFormView
    {
        #region Variables
        ...
        private String _dropZoneId = "SharedUserReqDropZone" + DateTime.Now.Ticks.ToString();
        ...
        private void GenerateFields()
        {
            ...
            List<RequirementFieldContract> lst = CurrentViewContext.RequirementItemFields.Take(_FieldsPerRow).ToList();

            // Maintains list of attributes that are already added
            List<Int32> lstTemporary = new List<Int32>();

            int _fieldsAdded = 0;
            for (int i = 1; i <= Math.Ceiling(Convert.ToDecimal(CurrentViewContext.RequirementItemFields.Count()) / _FieldsPerRow); i++)
            {
                if (_fieldsAdded == _FieldsPerRow)
                {
                    ...
                    foreach (var field in CurrentViewContext.RequirementItemFields)
                    {
        ...
        {
            System.Web.UI.Control fieldRow = Page.LoadControl("~\\ApplicantRotationRequirement\\UserControl\\SharedUserRequirementRowControl.ascx");
            (fieldRow as SharedUserRequirementRowControl).RequirementItemFields = lstAttributes;
            (fieldRow as SharedUserRequirementRowControl).NoOfFieldsPerRow = _FieldsPerRow;
            (fieldRow as SharedUserRequirementRowControl).ItemId = CurrentViewContext.ItemId;
            (fieldRow as SharedUserRequirementRowControl).TenantId = CurrentViewContext.TenantId;
            //Implemented code for UAT-708
            (fieldRow as SharedUserRequirementRowControl).CategoryId = CurrentViewContext.RequirementCategoryId;
            //(fieldRow as RequirementRowControl).PackageId = CurrentViewContext.Re;

            (fieldRow as SharedUserRequirementRowControl).IsFileUploadApplicable -= new EventHandler(RowControl_IsFileUploadApplicable);
            (fieldRow as SharedUserRequirementRowControl).IsFileUploadApplicable += new EventHandler(RowControl_IsFileUploadApplicable);

            (fieldRow as SharedUserRequirementRowControl).IsViewDocumnetApplicable -= new EventHandler(RowControl_IsViewDocumnetApplicable);
            (fieldRow as SharedUserRequirementRowControl).IsViewDocumnetApplicable += new EventHandler(RowControl_IsViewDocumnetApplicable);

            (fieldRow as SharedUserRequirementRowControl).IsViewVideoApplicable -= new EventHandler(RowControl_IsViewVideoApplicable);
            (fieldRow as SharedUserRequirementRowControl).IsViewVideoApplicable += new EventHandler(RowControl_IsViewVideoApplicable);
            (fieldRow as SharedUserRequirementRowControl).LstRequirementObjTreeProperty = LstRequirementObjTreeProperty;
            ...
            {
                (fieldRow as SharedUserRequirementRowControl).ApplicantRequirementItemId = CurrentViewContext.RequirementItemData.RequirementItemDataID;
                if (CurrentViewContext.RequirementItemData.ApplicantRequirementFieldData.IsNotNull())
                {
                    (fieldRow as SharedUserRequirementRowControl).ApplicantRequirementFieldData = CurrentViewContext.RequirementItemData.ApplicantRequirementFieldData.ToList();
                }
        ...
        private void BindItemPaymentData(Int32? orderID)
        {
            try
            {
                ...
                INTSOF.UI.Contract.ComplianceOperation.ItemPaymentContract oldData = SysXWebSiteUtils.SessionService.GetCustomData(ResourceConst.INSTRUCTOR_PRECEPTOR_PARKING_CART) as INTSOF.UI.Contract.ComplianceOperation.ItemPaymentContract;
