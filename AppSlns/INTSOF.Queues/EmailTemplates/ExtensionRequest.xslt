...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="mstns:ExtensionRequest" >
    <HTML>
      <BODY>
  ...
  <xsl:template match="mstns:Contents">
    <p>
      You are being informed of the decision to <xsl:value-of select="mstns:DecisionText"/> the <xsl:value-of select="mstns:ExtensionCategory"/>
      Extension Request <xsl:value-of select="mstns:ExtraText"/>for Service Order ID: <xsl:value-of select="mstns:SvcReqId"/> so that you may take any necessary next steps.
    </p>
