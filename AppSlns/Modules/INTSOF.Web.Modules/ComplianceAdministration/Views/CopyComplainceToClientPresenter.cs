...
{
    public class CopyComplainceToClientPresenter : Presenter<ICopyComplainceToClientView>
    {
        ...
        {
            List<GetRuleSetTree> lstRuleSetTree = ComplianceSetupManager.GetComplianceTree(View.TenantId).ToList();
        ...
        public void CopyToClient()
        {
            ...
            {
                String complianceDetails = BuildComplianceDetailsXML(lstElementsToAdd, lstElementsToRemove);
                ComplianceSetupManager.CopyComplianceToClient(complianceDetails, View.CurrentLoggedInUserId, View.ManageTenantId, View.TenantId);
            }
        ...
        public List<GetRuleSetTree> ManageParentChild(List<GetRuleSetTree> lstRuleSetTree, List<GetRuleSetTree> lstRuleSetTreeFromUI)
        {
            ...
            //List<GetRuleSetTree> lstParentNodes = tempList.Where(cond => cond.ParentNodeID == null).ToList();
        ...
        public List<String> ManageChildren(List<GetRuleSetTree> lstRuleSetTreeFromUI, List<GetRuleSetTree> tempList, GetRuleSetTree ruleSetTree, Boolean checkChild)
        {
            List<String> tempNodeIds = new List<string>();
            List<String> FinalNodeIDs = new List<string>();
            FinalNodeIDs.Add(ruleSetTree.NodeID);
            tempNodeIds = tempList.Where(cond=>cond.ParentNodeID == ruleSetTree.NodeID).Select(x=>x.NodeID).ToList();
            FinalNodeIDs.AddRange(tempNodeIds);
            while (tempNodeIds.Count() > 0)
            {
                FinalNodeIDs.AddRange(tempList.Where(cond => tempNodeIds.Contains(cond.ParentNodeID)).Select(x => x.NodeID));
                tempNodeIds = tempList.Where(cond => tempNodeIds.Contains(cond.ParentNodeID)).Select(x => x.NodeID).ToList();
            }
        ...
        private IntegrityCheckResponse CheckIfObjectCanBeRemoved(GetRuleSetTree ruleSetTree)
        {
            ...
            switch (ruleSetTree.UICode)
            {
                case RuleSetTreeNodeType.Package:
                    CompliancePackage compliancePackage = ComplianceSetupManager.GetPackageDetailsByCode(View.ManageTenantId, ruleSetTree.NodeCode);
                    if (compliancePackage.IsNotNull())
                    {
                        response = IntegrityManager.IfPackageCanBeUnassigned(compliancePackage.CompliancePackageID, compliancePackage.PackageName, View.ManageTenantId);
                    }
                    break;

                case RuleSetTreeNodeType.Category:
                    ComplianceCategory complianceCategory = ComplianceSetupManager.GetCategoryDetailsByCode(View.ManageTenantId, ruleSetTree.NodeCode);
                    CompliancePackage parentPackage = ComplianceSetupManager.GetPackageDetailsByCode(View.ManageTenantId, ruleSetTree.ParentNodeCode);
                    if (complianceCategory.IsNotNull() && parentPackage.IsNotNull())
                    {
                        response = IntegrityManager.IfCategoryCanBeUnassigned(parentPackage.CompliancePackageID, complianceCategory.CategoryName, View.ManageTenantId);
                    }
                    break;

                case RuleSetTreeNodeType.Item:
                    ComplianceItem complianceItem = ComplianceSetupManager.GetItemDetailsByCode(View.ManageTenantId, ruleSetTree.NodeCode);
                    ComplianceCategory parentCategory = ComplianceSetupManager.GetCategoryDetailsByCode(View.ManageTenantId, ruleSetTree.ParentNodeCode);
                    if (complianceItem.IsNotNull() && parentCategory.IsNotNull())
                    {
                        response = IntegrityManager.IfItemCanBeUnassigned(parentCategory.ComplianceCategoryID, complianceItem.Name, View.ManageTenantId);
                    }
                    break;

                case RuleSetTreeNodeType.Attribute:
                    ComplianceAttribute complianceAttribute = ComplianceSetupManager.GetAttributeDetailsByCode(View.ManageTenantId, ruleSetTree.NodeCode);
                    ComplianceItem parentItem = ComplianceSetupManager.GetItemDetailsByCode(View.ManageTenantId, ruleSetTree.ParentNodeCode);
                    if (complianceAttribute.IsNotNull() && parentItem.IsNotNull())
                    {
                        response = IntegrityManager.IfAttributeCanBeUnassigned(parentItem.ComplianceItemID, complianceAttribute.Name, View.ManageTenantId);
                    }
                    break;
        ...
        private String BuildComplianceDetailsXML(List<GetRuleSetTree> lstElementsToAdd, List<GetRuleSetTree> lstElementsToRemove)
        {
            String complianceDetailsXML = String.Empty;
           
            foreach (var element in lstElementsToAdd)
            {
                XElement root = new XElement("ComplianceDetail");
                XElement row = null;
                if (element.DataID.IsNotNull())
                {
                    row = new XElement("ComplianceID");
                    row.Value = element.DataID.ToString();
                    root.Add(row);
                }
                if (element.ParentDataID.IsNotNull())
                {
                    row = new XElement("ParentComplianceID");
                    row.Value = element.ParentDataID.ToString();
                    root.Add(row);
                }
                if (element.NodeCode.IsNotNull())
                {
                    row = new XElement("NodeCode");
                    row.Value = element.NodeCode.Value.ToString();
                    root.Add(row);

                }
                if (element.ParentNodeCode.IsNotNull())
                {
                    row = new XElement("ParentNodeCode");
                    row.Value = element.ParentNodeCode.Value.ToString();
                    root.Add(row);
                }
                if (element.UICode.IsNotNull())
                {
                    row = new XElement("UICode");
                    row.Value = element.UICode;
                    root.Add(row);
                }
                row = new XElement("IsDisAssociated");
                row.Value = false.ToString();
                root.Add(row);
                complianceDetailsXML += root.ToString(); 
            }

            foreach (var element in lstElementsToRemove)
            {
                XElement root = new XElement("ComplianceDetail");
                XElement row = null;
                if (element.DataID.IsNotNull())
