...
{
    public class RequirementNonCompliantSearchPresenter : Presenter<IRequirementNonCompliantSearch>
    {
        ...
        private void GetInviteeTypes()
        {
            View.lstInviteeType = LookupManager.GetSharedDBLookUpData<lkpInvitationSource>().Where(x => x.IsDeleted == false)
                   .Select(cond => new LookupContract()
                   {
                       ...
                       Code = cond.Code
                   }).Where(x => x.Code == InvitationSourceTypes.APPLICANT.GetStringValue()).OrderBy(x => x.Name).ToList();
        }
        ...
        public void GetRotationDetail()
        {
           ...
           // }
            ...
            resultData = ProfileSharingManager.GetRequirementNonComplaintSharesData(View.UserID, View.OrganizationUserId,
                               tenantIds, View.ClinicalRotationSearchContract, View.GridCustomPaging);
