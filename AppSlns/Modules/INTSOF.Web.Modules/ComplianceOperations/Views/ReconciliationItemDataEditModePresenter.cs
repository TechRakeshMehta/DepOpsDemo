...
{
    public class ReconciliationItemDataEditModePresenter : Presenter<IReconciliationItemDataEditModeView>
    {
        ...
        {
            Boolean IsReconciliationDataSaved = true;
            ...
            //UAT-3112:-
            if (!verificationDetailsContract.IsNullOrEmpty() && !verificationDetailsContract.lstApplicantData.IsNullOrEmpty())
            {
                if (View.AttemptedItemStatus == ApplicantItemComplianceStatus.Approved.GetStringValue()
                    && View.AttemptedItemStatus != View.ItemComplianceStatusCode
                    )
                {
                    List<Int32> lst = new List<int>();
                    foreach (var item in verificationDetailsContract.lstApplicantData)
                    {
                        lst.Add(item.ApplicantComplianceItemID);
                    }
                    string acid = string.Join(",", lst.Distinct());
