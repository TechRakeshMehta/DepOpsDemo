...
{
    public class ItemDataLoaderPresenter : Presenter<IItemDataLoaderView>
    {
        ...
        public List<UserCurrentAssignments> GetCurrentAssignments()
        {
            QueueMetaData _queueMetaData = QueueManagementManager.GetQueueMetaDataByCode(View.SelectedTenantId_Global, QueueMetaDataType.Verification_Queue_For_Admin.GetStringValue());
            return QueueManagementManager.GetUserCurrentAssignments(View.CurrentLoggedInUserId, Convert.ToInt32(_queueMetaData.QMD_BusinessProcessID), View.SelectedTenantId_Global);
        }
        ...
        public void GetNextActionForItems_Backup(List<ApplicantComplianceItemData> lstItemData, List<ListItemAssignmentProperties> lstAssignmentProperties, out List<ApplicantComplianceItemData> _lstNextReviewRecord,
            ...
            out List<ApplicantComplianceItemData> _lstIncompleteSameQueue)
        {
            ...
            if (!_lstIncompleteDifferentQueue.IsNullOrEmpty())
            {
                ...
                GenerateInputXmlIncompleteRecords_Backup(_lstIncompleteDifferentQueue);
            }

            if (!_lstItemsNotIncomplete.IsNullOrEmpty())
            {
                _getNextActions = true;
                GenerateInputXmlOther_Backup(_lstItemsNotIncomplete, false);
            }
            ...
            foreach (var _nextAction in _lstNextActions)
            {
                    ...
                    && _nextAction.NextAction == lkpQueueActionType.Next_Level_Review_Required.GetStringValue())
                {
                    ...
                    || (_nextAction.NextAction == lkpQueueActionType.Proceed_To_Next_Queue.GetStringValue() && View.WorkQueue != WorkQueueType.UserWorkQueue
        ...
        /// <param name="_lstNextQueueRecord"></param>
        /// <param name="_lstEscalationRecord"></param>
        public void GetNextActionForItems(List<ApplicantComplianceItemData> lstItemData, List<ListItemAssignmentProperties> lstAssignmentProperties,
                                          out List<ApplicantComplianceItemData> _lstNextQueueRecord, out List<ApplicantComplianceItemData> _lstIncompleteSameQueue)
        {
            Boolean _getNextActions = false;
            // Check for items which are having ApplicantComplianceItemId >  0, Exception Items and their status is getting changed
            List<ApplicantComplianceItemData> _lstItemsNotIncomplete = lstItemData.Where(itmData => itmData.ApplicantComplianceItemID > 0
        ...
        public void CallHandleAssignmentParallelTask(List<ApplicantComplianceItemData> lstNextQueueRecords, List<ApplicantComplianceItemData> lstNextReviewRecords,
                                                     List<ApplicantComplianceItemData> lstIncompleteSameQueue, List<ApplicantComplianceItemData> lstEscalateRecords,
                                                     List<ApplicantComplianceItemData> lstAlreadyEscalated, List<ApplicantComplianceItemData> lstVerificationCompleted)
        {
            ...
            #region Generate XML for Incomplete Items

            if (!lstIncompleteSameQueue.IsNullOrEmpty()) //  Generate XML of Items which are to be moved to Next Queue, with New Status and QueueId
            {
                _shouldRunParallelTask = true;

                if (View.WorkQueue == WorkQueueType.UserWorkQueue)
                {
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstIncompleteSameQueue
                        .Where(x => x.ReSetInitialReview == true && !x.IsIncompleteWithDocuments).ToList(), false, true, false));
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstIncompleteSameQueue
                       .Where(x => x.ReSetInitialReview == true && x.IsIncompleteWithDocuments).ToList(), false, true, false));
                }
                else
                {
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstIncompleteSameQueue
                        .Where(x => x.ReSetInitialReview == true && !x.IsIncompleteWithDocuments).ToList(), false, true, true));
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstIncompleteSameQueue
                       .Where(x => x.ReSetInitialReview == true && x.IsIncompleteWithDocuments).ToList(), false, true, true));
                }

                _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstIncompleteSameQueue.Where(x => x.ReSetInitialReview == false).ToList()
                    , true, false, false));
            }
            ...
            #region Generate XML for records which next review

            if (!lstNextReviewRecords.IsNullOrEmpty())  // Generate XML of Items which are to be moved to Next Review Level
            {
                _shouldRunParallelTask = true;

                if (View.WorkQueue == WorkQueueType.UserWorkQueue)
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstNextReviewRecords, false, false, false));
                else
                    _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstNextReviewRecords, false, true, true));
            }

            #endregion

            #region Generate XML for records that need escalation or Are already escalated

            if (!lstEscalateRecords.IsNullOrEmpty())
            {
                _shouldRunParallelTask = true;
                _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstEscalateRecords, true, false, false, true));
            }

            if (!lstAlreadyEscalated.IsNullOrEmpty())
            {
                _shouldRunParallelTask = true;
                _handleAssignmentXML.Append(GenerateInputXmlHandleAssignment(lstAlreadyEscalated, true, false, false)); // Confirm
            }
        ...
        /// <returns></returns>
        public Boolean ClearQueueRecords(List<ApplicantComplianceItemData> lstNextQueueRecords, List<ApplicantComplianceItemData> lstNextReviewRecords,
                                         List<ApplicantComplianceItemData> lstDeleteRecords, List<ApplicantComplianceItemData> lstReviewCompleted,
                                         List<ApplicantComplianceItemData> lstEscalationRecords, List<ApplicantComplianceItemData> lstEscalationResolved,
                                         List<ApplicantComplianceItemData> lstFinalEscalationResolved)
        {
            Boolean _shouldRunClearQueueRecords = false;
            ...
            _sbInputXML.Append("<Queues>");

            List<ApplicantComplianceItemData> _lstTemp = lstNextQueueRecords.Where(x => !x.IsIncompleteWithDocuments).ToList();
            if (!_lstTemp.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearRecords(_lstTemp, _lstQueueMetaData, false, false));
            }

            if (!lstNextReviewRecords.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearRecords(lstNextReviewRecords, _lstQueueMetaData, false, true));
            }
            if (!lstEscalationRecords.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearRecords(lstEscalationRecords, _lstQueueMetaData, true, false));
            }

            // Remove Incomplete items with Only documents attached to it
            List<ApplicantComplianceItemData> _lstTempDelete = lstDeleteRecords.Where
                                                            (x => !String.IsNullOrEmpty(x.CurrentStatusCode) &&
                                                              x.CurrentStatusCode != ApplicantItemComplianceStatus.Incomplete.GetStringValue()).ToList();
            if (!_lstTempDelete.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearRecords(_lstTempDelete, _lstQueueMetaData, false, false, false));
            }
            if (!lstReviewCompleted.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearRecords(lstReviewCompleted, _lstQueueMetaData, false, false, false));
            }

            /// Escalated items being Approved, Rejected, etc.
            if (!lstFinalEscalationResolved.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLAlreadyEscalatedRecords(lstFinalEscalationResolved, _lstQueueMetaData, false));
            }

            ///Escalated Items moving to new queue type
            if (!lstEscalationResolved.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLAlreadyEscalatedRecords(lstEscalationResolved, _lstQueueMetaData, true));
            }

            _sbInputXML.Append("</Queues>");

            if (_shouldRunClearQueueRecords)
                return QueueManagementManager.ClearQueueRecords(View.CurrentLoggedInUserId, Convert.ToString(_sbInputXML), View.SelectedTenantId_Global);
        ...
        public void SendRejectionMail(Dictionary<String, Object> data)
        {
            ...
            foreach (var excRejContract in lstExceptionRejectionContract)
            {
                ...
                if (!isCategory)
                {
                    ...
                    //UAT-4345 - Start
                    dictMailData.Add(EmailFieldConstants.CATEGORY_NAME, excRejContract.CategoryName);
                    dictMailData.Add(EmailFieldConstants.ITEM_NAME, excRejContract.ComplianceItemName);
                    //UAT-4345 - End
                }
        ...
        private void GenerateInputXmlOther_Backup(List<ApplicantComplianceItemData> lstItems, Boolean useNewStatus, Boolean isEscalationRecords = false)
        {
        ...
        /// <param name="lstItems"></param>
        /// <param name="useNewStatus">True for the items, which are getting mvoed to different queues.</param>
        private void GenerateInputXmlIncompleteRecords_Backup(List<ApplicantComplianceItemData> lstItems)
        {
            String _queueTypeCode = String.Empty;
                                                        ...
                                                        ApplicantComplianceItemData applicantComplianceItemData, List<ListItemAssignmentProperties> lstAssignmentProperties)
        {
            ...
            //UAT-3951
            if (!View.SelectedRejectionReasonIds.IsNullOrEmpty() && applicantComplianceItemData.ApplicantComplianceItemID > 0)
            {
                ComplianceDataManager.SaveRejectionReasonAuditHistory(View.SelectedTenantId_Global, View.SelectedRejectionReasonIds, applicantComplianceItemData.ApplicantComplianceItemID, View.CurrentLoggedInUserId);
            }
            ...
            if (isComplianceRequired)
            {
                ...
                //UAT-3112:-
                if (!applicantComplianceItemData.ApplicantComplianceItemID.IsNullOrEmpty())
                {
                    string acid = Convert.ToString(applicantComplianceItemData.ApplicantComplianceItemID);
                    ComplianceDataManager.SaveBadgeFormNotificationData(View.SelectedTenantId_Global, acid, null, null, View.CurrentLoggedInUserId);
                }
            }
        ...
        public Boolean UpdateCategoryOverrideData(DateTime? expirationDate, String catOverrideStatus, ApplicantItemVerificationData categoryOverrideData, String CategoryOverrideNotes)
        {
            String catStatus = String.Empty;
            ...
            ApplicantItemVerificationData applicantItemData = GetComplianceItemData();
            if (!applicantItemData.ApplicantCompCatId.IsNullOrEmpty())
            {
                categoryDataId = applicantItemData.ApplicantCompCatId.Value;
            }

            List<lkpCategoryComplianceStatu> _lstCategoryStatus = LookupManager.GetLookUpData<lkpCategoryComplianceStatu>(View.SelectedTenantId_Global);
            List<lkpCategoryExceptionStatu> _lstCategoryExceptionStatus = LookupManager.GetLookUpData<lkpCategoryExceptionStatu>(View.SelectedTenantId_Global);
            //UAT-1804: Not-Compliant to Compliant email should be sent upon admin override of the final category
            PackageSubscription ps = ComplianceDataManager.GetPackageSubscriptionByID(View.SelectedTenantId_Global, View.CurrentPackageSubscriptionID_Global);
            ...
            _status = ComplianceDataManager.UpdateCategoryOverrideData(View.SelectedTenantId_Global, categoryDataId, expirationDate, catStatusId, catOverrideStatusId, View.CurrentLoggedInUserId, categoryOverrideData.ComplianceCatId, View.CurrentPackageSubscriptionID_Global, CategoryOverrideNotes);
            EvaluatePostSubmitRules(null);

            //UAT-1804: Not-Compliant to Compliant email should be sent upon admin override of the final category
            if (!ps.ComplianceStatusID.IsNullOrEmpty() && ps.ComplianceStatusID.Value != 0
                    && ps.lkpPackageComplianceStatu.Code == ApplicantPackageComplianceStatus.Not_Compliant.GetStringValue())
            {
                String tenantName = String.Empty;
                Entity.Tenant tenant = SecurityManager.GetTenant(View.SelectedTenantId_Global);
                if (!tenant.IsNullOrEmpty())
        ...
        public void DeleteOverideComplianceStatus()
        {
            String catStatusApprovedCode = ApplicantCategoryComplianceStatus.Approved.GetStringValue();
            List<ApplicantComplianceItemData> appCmpItmData = ComplianceDataManager.DeleteOverideRuleStatus(View.SelectedTenantId_Global, View.CurrentViewContext.CurrentLoggedInUserId,
              View.SelectedComplianceCategoryId_Global, View.CurrentViewContext.CurrentPackageSubscriptionID_Global, catStatusApprovedCode);
            if (appCmpItmData.IsNotNull())
            {
                foreach (var index in appCmpItmData)
                {
                    EvaluatePostSubmitRules(index.ComplianceItemID);
                }
            }
        }
        ...
        //UAT-4390
        public ApplicantItemVerificationData GetComplianceItemData()
        {
            List<ApplicantItemVerificationData> lstApplicantItemData = ComplianceDataManager.GetApplicantDataForVerification(View.SelectedComplianceCategoryId_Global, View.CurrentPackageSubscriptionID_Global, View.SelectedTenantId_Global);
            Guid catExceptionItemCode = new Guid(AppConsts.WHOLE_CATEGORY_GUID);
            return lstApplicantItemData.Where(cond => cond.CatExceptionItemCode != catExceptionItemCode).FirstOrDefault();
        }
        ...
        {
            //UAT-1812
            List<ApplicantComplianceItemData> lstNotApprovedItems = new List<ApplicantComplianceItemData>();
            ...
            if (!lstItemsToDelete.IsNullOrEmpty() || !lstItemData.IsNullOrEmpty())
            {
                ...
                lstItemData.ForEach(itmChng =>
                {
                    if ((itmChng.AttemptedStatusCode != itmChng.CurrentStatusCode) && (itmChng.AttemptedStatusCode == ApplicantItemComplianceStatus.Not_Approved.GetStringValue()))
                    {
                        ...
                        //UAT-1812
                        lstNotApprovedItems.Add(itmChng);
                    }
                });
                ...
                if (!lstItemIdsToShuffle.IsNullOrEmpty())
                {
                    ...
                    if (!lstItemSeriesOfCategory.IsNullOrEmpty())
                    {
                        ...
                        {
                            ShuffleShotSeriesItemData(GetSeriesIDOfItem(lstItemSeriesOfCategory, itemId, itemIds));
                        });
                    }
                }
                //UAT-1812:Creation of an Approval/rejection summary for applicant logins
                View.lstNotApprovedItems = lstNotApprovedItems;
            }
        }

        private Int32 GetSeriesIDOfItem(List<ItemSery> lstItemSeriesOfCategory, Int32 compItemId, List<Int32> itemIds)
        {
        ...
        public List<ItemReconciliationAvailiblityContract> GetItemReconciliationAvailiblityStatus(String itemIDs)
        {
            return QueueManagementManager.GetItemReconciliationAvailiblityStatus(View.SelectedTenantId_Global, itemIDs, View.CurrentPackageSubscriptionID_Global);
        }

        public Dictionary<Int32, Boolean> CheckIfItemsAreInReconciliationProcess(List<Int32> itemDataIDs)
        {
            return SecurityManager.CheckIfItemsAreInReconciliationProcess(itemDataIDs, View.SelectedTenantId_Global);
        }

        public Boolean ClearReconciliationQueueRecords(List<ApplicantComplianceItemData> lstItemsWithZeroReconciliationReviewCount, List<ApplicantComplianceItemData> lstItemsForReviewsMatched
                                                        , List<ApplicantComplianceItemData> _lstItemsToDeleteUnderReconcillation)
        {
            Boolean _shouldRunClearQueueRecords = false;
            List<QueueMetaData> _lstQueueMetaData = LookupManager.GetLookUpData<QueueMetaData>(View.SelectedTenantId_Global);
            ...
            if (!lstItemsWithZeroReconciliationReviewCount.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearReconciliationRecords(lstItemsWithZeroReconciliationReviewCount, _lstQueueMetaData, true, false));
            }

            if (!lstItemsForReviewsMatched.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearReconciliationRecords(lstItemsForReviewsMatched, _lstQueueMetaData, true, true));
            }

            if (!_lstItemsToDeleteUnderReconcillation.IsNullOrEmpty())
            {
                _shouldRunClearQueueRecords = true;
                _sbInputXML.Append(GenerateInputXMLClearReconciliationRecords(_lstItemsToDeleteUnderReconcillation, _lstQueueMetaData, true, false));
            }
            ...
            if (_shouldRunClearQueueRecords)
            {
                //HttpContext.Current.Session["IsItemDeletedFromReconciliationQueue"] = true;
                return QueueManagementManager.ClearReconciliationQueueRecords(View.CurrentLoggedInUserId, Convert.ToString(_sbInputXML), View.SelectedTenantId_Global);
            }

            return false;
        }

        private String GenerateInputXMLClearReconciliationRecords(List<ApplicantComplianceItemData> lstItemData, List<QueueMetaData> lstQueueMetaData,
                                                   Boolean isResetReviewRequired, Boolean nextQueueApplicable = true)
        {
            StringBuilder _sbInputXML = new StringBuilder();
            foreach (var itemData in lstItemData)
            {
                ...
                if (_newQueueId.IsNotNull())
                    _sbInputXML.Append("<NewQueueID>" + _newQueueId + "</NewQueueID>");
                _sbInputXML.Append("<RecordID>" + itemData.ApplicantComplianceItemID + "</RecordID>");
                _sbInputXML.Append("<IsResetReviewRequired>" + isResetReviewRequired + "</IsResetReviewRequired>");
                _sbInputXML.Append("</QueueDetail>");
            }
            return Convert.ToString(_sbInputXML);
        }

        public void CallHandleAssignmentReconciliationParallelTask(List<ApplicantComplianceItemData> lstItemNeedToSendForRandomReview, List<ApplicantComplianceItemData> lstItemAlreadyRandomReview)
        {
            StringBuilder _handleAssignmentXML = new StringBuilder();
            ...
            /* Contains the record with data entry and need to move to next Queue + 
             * Attempt was different queue and should be moved to next queue */
            if (!lstItemNeedToSendForRandomReview.IsNullOrEmpty()) // Generate XML of Items which are to be moved to Next Queue, with New Status and QueueId
            {
                _shouldRunParallelTask = true;
                _handleAssignmentXML.Append(GenerateInputXmlHandleReconciliationAssignment(lstItemNeedToSendForRandomReview, false));
            }

            if (!lstItemAlreadyRandomReview.IsNullOrEmpty()) // Generate XML of Items which are to be moved to Next Queue, with New Status and QueueId
            {
                _shouldRunParallelTask = true;
                _handleAssignmentXML.Append(GenerateInputXmlHandleReconciliationAssignment(lstItemAlreadyRandomReview, false));
            }
            ...
            if (_shouldRunParallelTask) // Run the Parallel task, only if XML is generated for any of the Item types
                QueueManagementManager.RunParallelTaskHandleReconciliationAssignment(dicHandleAssignmentData, _loggerService, _exceptionService, View.SelectedTenantId_Global);
        }

        private String GenerateInputXmlHandleReconciliationAssignment(List<ApplicantComplianceItemData> lstItems, Boolean isResetReviewRequired)
        {
            StringBuilder _sbXML = new StringBuilder();
            String _queueTypeCode = String.Empty;
            List<QueueMetaData> _lstQueueMetaData = LookupManager.GetLookUpData<QueueMetaData>(View.SelectedTenantId_Global);
            ...
            foreach (var itemData in lstItems)
            {
                ...
                _queueTypeCode = GetQueueTypeCodeByStatus(itemData.CurrentStatusCode);
                if (itemData.ReconciliationMatchingStatus == ReconciliationMatchingStatus.Not_Matched.GetStringValue())
                {
                    _queueTypeCode = QueueMetaDataType.Reconciliation_Queue_For_Admin.GetStringValue();

                    itemData.StatusID = _lstItemComplianceStatus.FirstOrDefault(x => x.Code.Equals(itemData.CurrentStatusCode)).ItemComplianceStatusID;
                }
                else
                {
                    if (itemData.ReconciliationMatchingStatus == ReconciliationMatchingStatus.Matched.GetStringValue())
                    {
                        _queueTypeCode = GetQueueTypeCodeByStatus(itemData.AttemptedStatusCode);
                        itemData.StatusID = _lstItemComplianceStatus.FirstOrDefault(x => x.Code.Equals(itemData.AttemptedStatusCode)).ItemComplianceStatusID;
                    }
        ...
        /// <summary>
        /// Method to save "Not Approved" status in "ApplComplianceItemStatusHistory" table.
        /// </summary>
        /// <param name="lstRejectedItems"></param>
        public void SaveSeriesRejectedItemStatusHistory(List<ExceptionRejectionContract> lstRejectedItems)
        {
            if (!lstRejectedItems.IsNullOrEmpty())
            {
                List<ApplicantComplianceItemData> lstNotApprovedSeriesItmData = new List<ApplicantComplianceItemData>();
                List<Int32> itemIds = new List<Int32>();
                List<ItemSery> lstItemSeriesOfCategory = new List<ItemSery>();
                lstItemSeriesOfCategory = ComplianceDataManager.GetItemSeriesForCategory(View.SelectedTenantId_Global, View.SelectedComplianceCategoryId_Global);
                if (!lstItemSeriesOfCategory.IsNullOrEmpty())
                {
                    lstRejectedItems.Where(cnd => cnd.ItemID.HasValue).DistinctBy(cond => cond.ItemID).ForEach(item =>
                    {
                        GetSeriesIDOfItem(lstItemSeriesOfCategory, item.ItemID.Value, itemIds);
                    });
                }
                if (!View.lstNotApprovedItems.IsNullOrEmpty())
                {
                    lstNotApprovedSeriesItmData = View.lstNotApprovedItems.Where(x => itemIds.Contains(x.ComplianceItemID)).ToList();
                    if (!lstNotApprovedSeriesItmData.IsNullOrEmpty())
                    {
                        ComplianceDataManager.SaveSeriesRejectedItemStatusHistory(View.SelectedTenantId_Global, lstNotApprovedSeriesItmData, View.CurrentLoggedInUserId);
                    }
                }
            }
        ...
        public Boolean ReconcillationOverRideByClntAdmin(List<ApplicantComplianceItemData> lstItemsWithZeroReconciliationReviewCount)
        {
            Boolean _shouldRunClearQueueRecords = false;
            List<QueueMetaData> _lstQueueMetaData = LookupManager.GetLookUpData<QueueMetaData>(View.SelectedTenantId_Global);
            ...
            if (_shouldRunClearQueueRecords)
                return QueueManagementManager.ReconcillationOverRideByClntAdmin(View.CurrentLoggedInUserId, Convert.ToString(_sbInputXML), View.SelectedTenantId_Global);
        ...
        {
            //UAT-2618:
            ComplianceDataManager.UpdateIsDocAssociated(View.SelectedTenantId_Global, View.CurrentPackageSubscriptionID_Global, true, View.CurrentLoggedInUserId);
        }
        #endregion

        public void SendMailOnComplianceStatusChange()
        {
            PackageSubscription ps = View.PackageSubscriptionBeforeSaving;

            if (!ps.IsNullOrEmpty() && ps.PackageSubscriptionID > 0 && ps.ComplianceStatusID.Value != 0)
            {
                String tenantName = String.Empty;
                Entity.Tenant tenant = SecurityManager.GetTenant(View.SelectedTenantId_Global);
                if (!tenant.IsNullOrEmpty())
        ...
        #region UAT-3805
        public void SendItemDocNotificationToAgencyUser()
        {
            PackageSubscription ps = View.PackageSubscriptionBeforeSaving;

            if (!ps.IsNullOrEmpty() && ps.PackageSubscriptionID > 0)
            {
                Boolean isCategoryApproved = ps.ApplicantComplianceCategoryDatas.Any(cnd => cnd.ComplianceCategoryID == View.SelectedComplianceCategoryId_Global
                                                                                      && !cnd.IsDeleted
                                                                                      &&
                ...
                if (!isCategoryApproved)
                {
                    ...
                    itemDocRequestData.CategoryIds = Convert.ToString(View.SelectedComplianceCategoryId_Global);
                    ...
                    itemDocRequestData.PackageSubscriptionID = ps.PackageSubscriptionID;
                    ...
                    //ProfileSharingManager.SendItemDocNotificationOnCategoryApproval(View.SelectedTenantId_Global, Convert.ToString(View.SelectedComplianceCategoryId_Global)
                    //                                                               , View.SelectedApplicantId_Global, String.Empty, lkpUseTypeEnum.COMPLIANCE.GetStringValue()
                    //                                                               , ps.PackageSubscriptionID, null);
                }
            }
        }
        #endregion
